...
/* Pastikan checkbox terpusat di sel */
.table td.text-center {
    padding: 8px 4px;
    width: 50px;
}
...
$(document).ready(function() {
    ...
    }).buttons().container().appendTo('#tableMateri_wrapper .col-md-6:eq(0)');
    ...
    $(document).on('click', '.badge', function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                console.log('Data:', {status: newStatus, <?= csrf_token() ?>: '<?= csrf_hash() ?>'});
    ...
    tableMateriPilihan.on('draw', function() {
        $('.table td.text-center input[type="checkbox"]').css({
            'margin': '0 auto',
            'display': 'block'
        });
    });
