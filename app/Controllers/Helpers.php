...
class Helpers extends BaseController
{
    ...
    /**
     * Serve CSS helper files from app/Helpers/css/
     * 
     * @param string $filename Nama file CSS helper
     * @return \CodeIgniter\HTTP\ResponseInterface
     */
    public function css($filename)
    {
        // Security: hanya izinkan file .css dengan karakter alphanumeric, underscore, dan dash
        if (!preg_match('/^[a-zA-Z0-9_-]+\.css$/', $filename)) {
            throw new \CodeIgniter\Exceptions\PageNotFoundException('Invalid filename');
        }
        
        $filePath = APPPATH . 'Helpers/css/' . $filename;
        
        if (!file_exists($filePath)) {
            throw new \CodeIgniter\Exceptions\PageNotFoundException('File not found: ' . $filename);
        }
        
        // Set proper headers untuk CSS dengan MIME type yang benar
        $this->response->setContentType('text/css; charset=utf-8');
